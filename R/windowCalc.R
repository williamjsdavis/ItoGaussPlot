stdWindowSlopeCast <- function(timeIn, ampIn, windows) {
  ...
  Nt <- length(timeIn)
  equalTsteps <- diff(range(diff(timeIn))) < .Machine$double.eps ^ 0.5
  if (equalTsteps) {
    dt <- time[2] - time[1]
    stepHandle <- function(w) stdWindowSlopeUniform(timeIn, ampIn, w, dt, Nt)
  } else {
    stepHandle <- function(w) stdWindowSlopeNonUniform(timeIn, ampIn, w, Nt)
  }
...
stdWindowSlopeUniform <- function(timeIn, ampIn, windowLength, dt, Nt) {
...
stdWindowSlopeNonUniform <- function(timeIn, ampIn, windowLength, Nt) {
